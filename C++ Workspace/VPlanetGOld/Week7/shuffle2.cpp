  ...
  if(last < M){
    ...
    for(int i = 1; i <= M;i ++){
      ...
      if(dist[i] <= N-M+1){
        ...
        int pos = cnt+withDist[dist[i] - (N-M+1)];
        assert(pos <= N);
        ans[pos] = i;
        //assert(N- M + stepsAway[N-i+1]+1 <= N); 
        //cerr << i << " " << stepsAway[N-i+1]+1 << '\n';
      }
    }
    for(int i = M+1; i <= N;i ++){
