...
void go(int r1, int c1, int p1, int r2, int c2, int p2, bool turn){
    assert(r1 >= 1 && r1 <= N && r2 >= 1 &&r2 <= N);
    assert(c1 >= 1 && c1 <= col[r1] && c2 >= 1 && c2 <= col[r2]);
    ...
    if(turn){
        //bool comeFrom = false;
        if(!bad[r1-1][c1-1] && r1 == c1 && r1 % 2 == 0 && r1 -1 >= 1 && c1-1 >= 1 && c1 <= col[r1-1]){
            wentSomewhere = true;
    ...
    if(!turn){
        if(!bad[r2-1][c2-1] && r2 == c2 && r2 % 2 == 0 && r2 - 1 >= 1 && c2 -1  >= 1 && c2 <= col[r2-1]){
            wentSomewhere = true;
...
int main(){
    ...
    while(t--){
        ...
        go(rA, pA, !bad[rA][pA], rB, pB, 1, !bad[rB][pB]);
        cout << "Case #" << ++tc << ": " << ans << "\n";
